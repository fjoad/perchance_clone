...
class TextGenerationService:
    ...
    def _generate(self, messages: list[dict[str, str]], *, max_new_tokens: int, temperature: float) -> str:
        if settings.use_mock_text:
    ...
    def chat_reply(
        ...
        if settings.use_mock_text:
            return (
                f"{character['display_name']} leans in with a small, knowing smile. "
                "This is a mock response path, but the app loop is working."
            )
        compiled = prompts.build_chat_messages(character, pinned_memory, summary, messages)
        return self._generate(compiled, max_new_tokens=280, temperature=0.8)
    ...
    def summarize_memory(
        ...
        compiled = prompts.build_summary_messages(character, pinned_memory, previous_summary, messages)
        return self._generate(compiled, max_new_tokens=140, temperature=0.2)
    ...
    def extract_scene(
        ...
        compiled = prompts.build_scene_messages(character, messages, note)
        raw = self._generate(compiled, max_new_tokens=160, temperature=0.3)
    ...
    def compose_image_prompts(
        ...
        if settings.use_mock_text:
            return prompts.fallback_image_prompts(character, scene_summary)
        compiled = prompts.build_image_prompt_messages(character, scene_summary)
        raw = self._generate(compiled, max_new_tokens=220, temperature=0.35)
        ...
        if not positive:
            return prompts.fallback_image_prompts(character, scene_summary)
        if not negative:
            negative = prompts.fallback_image_prompts(character, scene_summary)[1]
