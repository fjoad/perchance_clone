param(
    ...
    [int]$NumPredict = 520,
    [int]$NumCtx = 0,
    [string]$ImagePromptMode = "model_call"
)
...
function Stop-BenchmarkProcesses {
    $targets = Get-CimInstance Win32_Process | Where-Object {
        ...
        ($_.Name -eq "python.exe" -and $_.CommandLine -match "stable-diffusion-webui|bench_long_flow_a1111|launch.py") -or
...
try {
    ...
    $benchArgs = @(
        "scripts\bench_long_flow_a1111.py",
        ...
        "--num-ctx", "$NumCtx",
        "--image-prompt-mode", $ImagePromptMode
    )
