...
function Stop-BenchmarkProcesses {
    $targets = Get-CimInstance Win32_Process | Where-Object {
        ...
        ($_.Name -eq "python.exe" -and $_.CommandLine -match "stable-diffusion-webui|bench_text_model_policy_a1111|launch.py") -or
...
try {
    ...
    $benchArgs = @(
        "scripts\bench_text_model_policy_a1111.py",
    ...
    foreach ($model in $TextModel) {
        $benchArgs += @("--text-model", $model)
    }
