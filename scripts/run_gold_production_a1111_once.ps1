param(
    [string]$SampleDir = "outputs\research_gold_samples\echidna",
...
function Stop-BenchmarkProcesses {
    $targets = Get-CimInstance Win32_Process | Where-Object {
        ...
        ($_.Name -eq "python.exe" -and $_.CommandLine -match "stable-diffusion-webui|bench_gold_production_a1111|launch.py") -or
...
try {
    ...
    $benchArgs = @(
        "scripts\bench_gold_production_a1111.py",
        ...
        "--image-prompt-tokens", "$ImagePromptTokens",
        "--num-ctx", "$NumCtx"
    )
